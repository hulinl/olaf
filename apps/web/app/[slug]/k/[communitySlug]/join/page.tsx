...
/**
 * Dedikovaná stránka pro žádost o vstup do public komunity.
 * Mirror workspace `/[slug]/join/page.tsx` — landing má jen tlačítko,
 * celý flow žije tady.
 */
export default function CommunityJoinPage({ params }: Props) {
  const { slug, communitySlug } = use(params);
    ...
    (async () => {
      try {
        const [me, c] = await Promise.all([
          auth.me().catch(() => null),
          communitiesApi.detail(slug, communitySlug),
        ]);
        if (cancelled) return;
        setUser(me);
        setCommunity(c);
  ...
  async function submitJoin(
    ...
    setError(null);
    try {
      const res = await communitiesApi.join(slug, communitySlug, account);
      ...
      if (
        ...
        router.replace(
          `/login?next=/${slug}/k/${communitySlug}/join&email=${emailQ}`,
        );
        return;
      }
      setError(
  ...
  if (community.visibility !== "public") {
    router.replace(`/${slug}/k/${communitySlug}`);
    return null;
  }

  const backHref = `/${slug}/k/${communitySlug}`;
  ...
  const memberStatus = community.my_membership?.status;

  if (submitted === "already_member" || memberStatus === "member") {
    return (
    ...
    return (
      <div className="bg-canvas text-ink-900">
        ...
        <main className="mx-auto flex max-w-3xl flex-col items-center px-4 py-16 text-center">
          ...
          <p className="mt-3 max-w-md text-ink-700">
            {submitted === "pending"
              ? `Poslali jsme ti potvrzení na e-mail. Až tvojí žádost správce komunity „${community.name}" schválí, dáme vědět.`
              : `Tvoji žádost teď posuzuje správce komunity „${community.name}". Až rozhodne, přijde ti upozornění.`}
          </p>
      ...
      <div className="bg-canvas text-ink-900">
        ...
        <main className="mx-auto flex max-w-3xl flex-col px-4 py-12">
          <div className="mx-auto w-full max-w-lg">
            <h1 className="text-3xl font-semibold tracking-tight text-ink-900">
              Přidej se do komunity {community.name}
            </h1>
  ...
  const nextParam = encodeURIComponent(`/${slug}/k/${communitySlug}/join`);
  return (
    <div className="bg-canvas text-ink-900">
      {header}
      <main className="mx-auto flex max-w-3xl flex-col px-4 py-12">
          ...
          <Card className="mt-6">
            <CardSection>
              <div className="flex flex-col gap-3">
                <LinkButton
                  href={`/signup?next=/${slug}/k/${communitySlug}/join`}
                  variant="primary"
                  size="lg"
                  fullWidth
                >
          ...
          {guestOpen && (
            <Card className="mt-4">
              <CardSection>
                ...
                >
                  <Field label="E-mail" htmlFor="cjoin-email">
                    <Input
                      id="cjoin-email"
                      type="email"
                      required
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <Field label="Jméno" htmlFor="cjoin-first">
                      <Input
                        id="cjoin-first"
                        required
                    ...
                    <Field label="Příjmení" htmlFor="cjoin-last">
                      <Input
                        id="cjoin-last"
                        required
