    ...
    (async () => {
      try {
        ...
        if (ws.my_role !== "owner" && ws.my_role !== "admin") {
          try {
            await auth.me();
            // Member (ne-admin) tady nemá co dělat — /tvurce/komunity je
            ...
            router.replace(`/login?next=/tvurce/komunity/${slug}`);
          }
          return;
        }
        setWorkspace(ws);
    ...
    <div className="flex flex-col gap-6">
      ...
      <header className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <LinkButton
            href={`/tvurce/komunity/${workspace.slug}/edit`}
            variant="secondary"
            size="md"
          >
            Upravit komunitu →
...
function EventMobileCard({
  ...
  const href = `/tvurce/akce/${wsSlug}/${event.slug}`;
