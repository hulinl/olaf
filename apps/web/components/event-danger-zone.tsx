...
export function EventDangerZone({ event, workspaceSlug, onCancelled }: Props) {
  ...
  const deleteBlock = (
    <Card className="border-danger/30">
      <CardSection>
        <h2 className="text-base font-semibold text-danger">Smazat akci</h2>
        <p className="mt-1 text-sm text-ink-700">
          Akce zmizí z přehledů a veřejné stránky, ale 30 dní zůstane v
          koši — můžeš ji vrátit. Po 30 dnech se smaže napořád.
          {event.confirmed_count > 0 && (
            <>
              {" "}
              <strong>Pozor:</strong> {event.confirmed_count}{" "}
              {event.confirmed_count === 1
                ? "potvrzená přihláška"
                : event.confirmed_count < 5
                  ? "potvrzené přihlášky"
                  : "potvrzených přihlášek"}{" "}
              ztratí přístup ke stránce.
            </>
          )}
        </p>
        ...
        {!deleteOpened ? (
          ...
          <div className="mt-5 flex flex-col gap-4 rounded-md border border-danger/30 bg-danger-soft/40 p-4">
            <p className="text-sm text-ink-900">
              Opravdu poslat „{event.title}" do koše? Můžeš ji vrátit do 30
              dní.
            </p>
      ...
      <div className="flex flex-col gap-4">
        <Card className="border-danger/30">
          <CardSection>
            ...
            <p className="mt-1 text-sm text-ink-700">
              Stav: <strong>cancelled</strong>. Všem registrovaným byl
              odeslán zrušovací email.
            </p>
    ...
    <div className="flex flex-col gap-4">
      <Card className="border-danger/30">
        <CardSection>
          ...
          <p className="mt-1 text-sm text-ink-700">
            Po zrušení dostanou všichni přihlášení i lidé na waitlistu email
            s informací (a tvým důvodem, pokud ho napíšeš). Veřejná stránka
            akce zůstane viditelná, ale s banner „ZRUŠENO".
          </p>
